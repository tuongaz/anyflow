...
- Movable nodes (US-014): React Flow's `nodesDraggable` is enabled when `onNodePositionChange` is wired (otherwise read-only). DemoCanvas only fires the parent callback on `onNodeDragStop` — drag-while-moving stays in React Flow's internal state so we don't PATCH per pixel. DemoView keeps a `positionOverrides: Record<id, {x,y}>` map; on drag-stop it sets the override (optimistic), PATCHes `/api/demos/:id/nodes/:nodeId/position`, then either lets the next `demo:reload` echo prune the override (server caught up) or, on failure, drops the override and surfaces a `data-testid='position-error-banner'`. The watcher echo is intentional — pruning override entries that match the server position prevents both the loop and any visual flicker. An editor-driven position change still wins because the override is cleared once the server matches the local copy.
