...
- The studio mutates user files in exactly two places: (1) `.anydemo/sdk/emit.ts` writes during `register` (only when the demo declares an event-bound state node) via `apps/studio/src/sdk-writer.ts`; (2) `PATCH /api/demos/:id/nodes/:nodeId/position` rewrites `.anydemo/demo.json` on node drag-stop via `apps/studio/src/api.ts`. Position writes go through `writeFileAtomic` (tempfile + `rename(2)`) and are serialized per demoId via `withDemoWriteLock` so concurrent drags can't race the read-modify-write sequence. Output is `JSON.stringify(obj, null, 2) + '\n'` — 2-space indent + trailing newline.
...
- Movable nodes (US-014 + US-021): React Flow's `nodesDraggable` is enabled when `onNodePositionChange` is wired (otherwise read-only). DemoCanvas only fires the parent callback on `onNodeDragStop` — drag-while-moving stays in React Flow's internal state so we don't PATCH per pixel. DemoView holds two `usePendingOverrides<T>()` hooks (`apps/web/src/hooks/use-pending-overrides.ts`) — one for nodes, one for connectors. On drag-stop it `setOverride(nodeId, { position })` (optimistic), PATCHes `/api/demos/:id/nodes/:nodeId/position`, then either lets the next `demo:reload` echo `pruneAgainst(snapshot)` per-field (server caught up) or, on failure, `dropOverride(nodeId)` and surfaces a `data-testid='edit-error-banner'`. The watcher echo is intentional — per-field pruning of overrides that match the server prevents both the loop and any visual flicker. An editor-driven position change still wins because the override is cleared once the server matches the local copy. Pure reducers `applySetOverride/applyDropOverride/applyPruneAgainst` are exported alongside the hook so they can be unit-tested without a React renderer (none is installed in apps/web). Use this hook for every new optimistic UI mutation — don't add ad-hoc `useState` maps.
