...
export type ViewStateNodeType = Node<NodeData & Record<string, unknown>, 'stateNode'>;

const HANDLE_STYLE: CSSProperties = { opacity: 0, pointerEvents: 'none' };
const DEFAULT_W = 200;

export function ViewStateNode({ data }: NodeProps<ViewStateNodeType>) {
  ...
  const containerStyle: CSSProperties = {
    ...
    ...(data.borderStyle ? { borderStyle: data.borderStyle } : { borderStyle: 'dashed' }),
