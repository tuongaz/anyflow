 ...
 * `.anydemo/sdk/emit.ts` by `anydemo register` when the demo declares any
 * event-bound state node. Kept dependency-free so the user's app can call it
 * without installing `@anydemo/sdk`.
 */
export const EMIT_TEMPLATE = `// Auto-generated by anydemo register. Safe to commit; safe to delete.
...
export async function emit(
  ...
  const base = (
    opts.studioUrl ?? readEnv('ANYDEMO_STUDIO_URL') ?? 'http://localhost:4321'
  ).replace(/\\/+$/, '');
